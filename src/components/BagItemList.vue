...
<!-- <template>
	<div>
		<h2>Bag Items</h2>
		<div v-for="item in bag.items" :key="item.title">
			{{ item.title }}
			<button @click="removeFromBag(item)">Remove from Bag</button>
		</div>
		<input v-model="checkoutName" placeholder="Enter your name" />
...
<template>
	<div>
		...
		<div class="row row-cols-1 row-cols-sm-2 row-cols-lg-3 row-cols-xxl-4 g-1 justify-content-center">
